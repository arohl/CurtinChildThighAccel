...
def load_data(datafile, acc_prefix, check=True, ignore_labels=None):
    ...
    data = pd.read_csv(
        datafile,
        index_col="global_time",
        parse_dates=["global_time"],
    ...
    if check:
        time_diffs = (
            pd.Series(data.index).diff().dt.total_seconds() * 1000
        )  # convert to ms
        ...
        if pd.Series(data.index).is_monotonic_increasing:
            ...
            if len(freq_counts) == 1:
                ...
                print(
                    " ".join(
                        [
                            f"(Δ: {diff:.1f} ms, count: {count})"
                            for diff, count in freq_counts.items()
                        ]
                    )
                )
                ...
                print(
                    f"Average: {mean_diff:.2f} ms Standard deviation: {std_diff:.2f} ms"
                )
        else:
...
def make_windows(
    ...
    for t, w in tqdm(
        data.groupby(
            pd.Grouper(freq=f"{winsec}s", origin="start", closed="left", dropna=False)
        ),
        disable=verbose,
    ):
        frame += 1
        ...
        with warnings.catch_warnings():
            ...
            dominant_label_percentage = (
                label_counts.max() / len(w["annotation"])
            ) * 100
            if drop_impure:
                    ...
                    if verbose:
                        if frame_info:
                            if is_good_window(x, t, frame, sample_rate, winsec):
                                print(
                                    f"Frame {frame} has {dominant_label_percentage:.2f}% {dominant_label} labels but impure"
                                )
                            else:
                                print(
                                    f"Frame {frame} has {dominant_label_percentage:.2f}% {dominant_label} labels but impure and not good"
                                )
        ...
        if not is_good_window(x, t, frame, sample_rate, winsec):  # skip if bad window
            if verbose:
                if frame_info:
                    print(
                        f"Frame {frame} has {dominant_label_percentage:.2f}% {dominant_label} labels but not good"
                    )
        ...
        if frame_info:
            print(
                f"Frame {frame} has {dominant_label_percentage:.2f}% {dominant_label} labels"
            )
        X.append(x)
        Y.append(y)
        T.append(t)
    ...
    if verbose:
        ...
        if impure_windows:
            print(
                f"{len(impure_windows)} impure windows dropped at frames {impure_windows}"
            )
        if len(X) > 0:
...
def is_good_window(x, t, frame, sample_rate, winsec):
    ...
    if len(x) < window_len:
        ...
        if len(x) == window_len + 1:
            print(
                f"WARNING: Window length of {len(x)} is greater than the expected value of {window_len} at frame {frame}"
            )
            print(f"Timestamp of start of this window is {t}")
            return False
        else:
            raise ValueError(
                f"Window length of {len(x)} is greater than the expected value of {window_len}"
            )
...
def map_to_new_classes(df, column_name, mapping_file, verbose=True):
    ...
    if verbose:
        ...
        if len(mismatched_values) > 0:
            print(
                f"The class map does not contain the following values: {mismatched_values}"
            )
        ...
        if len(mismatched_values) > 0:
            print(
                f"The class map contains the following values that are not the dataset: {mismatched_values}"
            )
...
def plot_confusion_matrix(
    y_test, y_pred, label_encoder, original_labels, figsize=(20, 15)
):
    """
    Plots a confusion matrix with percentages and counts.

    Parameters:
    - y_test: Array of true labels.
    - y_pred: Array of predicted labels.
    - label_encoder: A label encoder to inverse transform the encoded labels.
    ...
    cm = confusion_matrix(y_test_decoded, y_pred_decoded, labels=original_labels)

    # Calculate the percentages
    cm_percentage = cm / cm.sum(axis=1)[:, np.newaxis]

    # Get class labels
    class_labels = np.unique(np.concatenate((y_test_decoded, y_pred_decoded)))

    # Plot the confusion matrix with number and percentage
    plt.figure(figsize=figsize)
    sns.heatmap(
        cm_percentage,
        annot=False,  # Show the percentages
        fmt=".1%",  # Format percentages to one decimal place
        cmap="Blues",
        xticklabels=class_labels,
        yticklabels=class_labels,
    )

    # Loop to add the numbers on top of the percentages
    for i in range(len(class_labels)):
        for j in range(len(class_labels)):
            # Set text color to white for true positives, black for others
